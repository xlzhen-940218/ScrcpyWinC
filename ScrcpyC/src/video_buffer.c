...
run_buffering(void *data) {
    ...
    for (;;) {
        ...
        assert(!sc_queue_is_empty(&vb->b.queue));
        *(&vb_frame) = (&vb->b.queue)->first;
        (&vb->b.queue)->first = (&vb->b.queue)->first->next;
...
#endif
    vb_frame->next = NULL;
    if (sc_queue_is_empty(&vb->b.queue)) {
        vb->b.queue.first = vb->b.queue.last = vb_frame;
    }
    else {
        vb->b.queue.last->next = vb_frame;
        vb->b.queue.last = vb_frame;
    }
